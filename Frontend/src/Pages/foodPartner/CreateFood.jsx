...
const CreateFood = () => {
    ...
    try {
        ...
        console.log('⚠️ Upload attempt completed with:', error.message);
        
        // If it's just a browser extension error, consider it successful
        if (error.message.includes('message channel closed') || 
            error.message.includes('asynchronous response')) {
            uploadSuccess = true;
            console.log('🔧 Treating extension error as success');
        } else {
            setFileError(error.response?.data?.msg || 'Failed to upload video');
        }
    } finally {
        // 🔥 ALWAYS REDIRECT AFTER UPLOAD ATTEMPT
        if (uploadSuccess || !fileError) {
